...
if ($si->vars['bid']) {
	//$blog = $content->fetch(sprintf("SELECT * FROM blogs WHERE bid='%s'", $si->vars['bid']));
        $blog = $content->query(sprintf("SELECT * FROM blogs WHERE bid='%s'", $si->vars['bid']));
        $blog = $content->fetch($blog, false);
	...
	// show the blog
        if ($blog['uid'] == $si->sess[$sessname]) {
            $temp->fetch_temp('page_blog_own');
        } else {
            $temp->fetch_temp('page_blog');
        }
	
	// this is for comments...
	//$temp->fetch_temp('page_comments');
        unset($accessed);
        // comments are gone!
	/*$result = $content->query(sprintf("SELECT rowid, comment, uid, created FROM comments WHERE bid='%s'", $si->vars['bid']));
        while ($comment = $content->fetch($result, false)) {
		...
		while ($reply = $result2->fetchArray()) {
			...
			$replies .= $temp->fetch_temp('page_reply', true);
...
// load the blogs
	...
	if ( (int)$naught == $naught) {
		...
		//if ( $result->numRows() > 0 ) {
                if ($content->query(sprintf("SELECT COUNT(bid) FROM blogs WHERE bid='%s'", $si->vars['start'])) >= 1) {
...
if (!$si->vars['bid']) {
    while ($blogs = $content->fetch($blog, false)) {
        ...
        // we need to set the query back to $blog
        // since we changed it when we called $user
        $content->setQuery($blog);
